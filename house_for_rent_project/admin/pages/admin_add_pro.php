...
<label for="main_image" class="control-label">Add more images<span style="color:red; font-size:20px;">&nbsp;<strong>*</strong></span></label>
  <input type="file" class="form-number required form-control"  required style="padding:0px;" name="images[]" >           
  </div>
                      </div>
  </div>
                  </fieldset>
  ...
  <button type="submit" class="btn btn-success btn-default">Cancel</button>
            ...
            if(isset($_POST['submit']))
            {
              ...
              $image = $_FILES['image']['name'];
               ...
               $sql = $conn->query("INSERT into images (name,main,post) values($image,'yes',$lastId)") or trigger_error($conn->error);
    ...
    if(!empty(array_filter($_FILES['files']['name']))){
        foreach($_FILES['images']['name'] as $key=>$val){
            ...
            if(in_array($fileType, $allowTypes)){
                ...
                if(move_uploaded_file($_FILES["files"]["tmp_name"][$key], $targetFilePath)){
                    // Image db insert sql
                  $propertyId=""
                    $insertValuesSQL .= "('".$fileName."','no',$lastId";
                }else{
        ...
        if(!empty($insertValuesSQL)){
            ...
            $insert = $db->query("INSERT INTO images (name,main) VALUES $insertValuesSQL");
            if($insert){
